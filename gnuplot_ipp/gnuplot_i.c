...
/**
  @brief    Plots a 2d surface plot from a list of floats.
  @param    handle  Gnuplot session control handle.
  @param    d       Array of doubles.
  @param    n       Number of values in the x-dir passed array.
  @param    m       Number of values in the y-dir passed array.
  @param    title   Title of the plot.
  @return   void

  Plots out a 2d graph from a list of floats. The x-coordinate is the
  index of the floats in the list, the y coordinate is the floats in
  the list.
  ...
  @code
    ...
    gnuplot_splot(h, d, 50, 50, "surface") ;
    sleep(2) ;
    gnuplot_close(h) ;
  @endcode
 */
...
)
{
    ...
    if (handle==NULL || d==NULL || (n<1) || (m<1)) return ;
...
{
  double dx = (xmax-xmin)/m, dy = (ymax-ymin)/n;
    ...
    for (i=0 ; i<m ; i++) {
      for (j=0 ; j<n ; j++) {
        fprintf(tmpfd, "%e %e %.18e\n", xmin+dx*i, ymin+dy*j,d[i*n+j]);
      }
      fprintf(tmpfd,"\n");
    }
    fclose(tmpfd) ;

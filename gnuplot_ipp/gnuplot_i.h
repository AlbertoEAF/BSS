...
/* ----------------------- splot extension ------------------------- */
// Adapted from : http://diffusionht.blogspot.pt/2012/08/adding-splot-surface-plot-interface-to.html


void gnuplot_splot_atmpfile(gnuplot_ctrl * handle, char const* tmp_filename, char const* title);

// Assumes the 2D data is packed in rows on a 1D array as [..row0..,..row1..,...]
void gnuplot_splot(
    gnuplot_ctrl    *   handle,
    double          *   d,
    int                 n,
...
/// True splot with the true x-y ranges and not indexes

...
{
 public:
  Gnuplot(const std::string & style = "lines")  { h = gnuplot_init ( ); if (style.length()) setstyle(style.c_str()); }
  ...
  // Implicit plot choices

  void plot (double *y, int n, const char *title) { gnuplot_plot_x(h,y,n,title); }
  void plot (double *x, double *y, int n, const char *title) { gnuplot_plot_xy(h,x,y,n,title);}

  void replot (double *y, int n, const char *title) { reset(); plot_y(y,n,title); }
  void replot (double *x, double *y, int n, const char *title) { reset(); plot_xy(x,y,n,title); }


  // Explicit plot type choice
  void plot_y   (double *y, int n, const char *title) { gnuplot_plot_x(h,y,n,title); }
  void plot_xy  (double *x, double *y, int n, const char *title) { gnuplot_plot_xy(h,x,y,n,title);}

  void replot_y (double *y, int n, const char *title) { reset(); plot_y(y,n,title); }
  ...
  void splot(double *d, int cols, int rows, const char *title) { gnuplot_splot(h, d, cols, rows, title); }

  void splot(double *d, int cols, int rows, double xmin, double xmax, double ymin, double ymax, const char *title) { gnuplot_splot_ranged(h, d, cols, rows, xmin, xmax, ymin, ymax, title); }
